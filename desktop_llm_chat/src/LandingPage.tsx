...
export default function LandingPage({ onEnter }: { onEnter: () => void }) {
  return (
    <div style={{ background: '#0b0d0c', minHeight: '100vh', color: '#fff', fontFamily: "'Inter', sans-serif", overflow: 'auto' }}>
      ...
      <header style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 28px', background: 'rgba(11,13,12,0.85)', backdropFilter: 'blur(20px)', borderBottom: '1px solid #1e2a24' }}>
        ...
        <button onClick={onEnter} style={{ background: 'transparent', border: '1px solid #1e2a24', color: '#fff', padding: '8px 20px', borderRadius: 10, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>Open Chat →</button>
      </header>
      ...
      <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
          ...
          <button onClick={onEnter}
            style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '15px 36px', background: '#3bf185', color: '#0a2f1a', fontWeight: 700, fontSize: 15, border: 'none', borderRadius: 14, cursor: 'pointer' }}>
      ...
      <section style={{ padding: '0 24px 80px', maxWidth: 800, margin: '0 auto' }}>
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <button onClick={onEnter}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '15px 36px', background: '#3bf185', color: '#0a2f1a', fontWeight: 700, fontSize: 15, border: 'none', borderRadius: 14, cursor: 'pointer', position: 'relative' }}>
